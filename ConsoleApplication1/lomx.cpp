...
{
	// x3.xem+235BD
	return very_real_unpack(ctx->table, ctx->tablesz, ctx->tre, ctx->allocsz, ctx->firstbyte,
		src, ssize, dst, dsize);
}
...
#pragma endregion write dos header to file
			...
			// write section
			aImageSectionHeader = reinterpret_cast<IMAGE_SECTION_HEADER*>(
				reinterpret_cast<PBYTE>(pImageNtHeader)+sizeof(DWORD) + sizeof(IMAGE_FILE_HEADER) + pImageNtHeader->FileHeader.SizeOfOptionalHeader);
			...
			for (int i = 0; i < pImageNtHeader->FileHeader.NumberOfSections; i++)
			{
				...
				WriteFile(hFile, ((PBYTE)pFile) + pXModule->header.fix + aImageSectionHeader[i].PointerToRawData,
					aImageSectionHeader[i].SizeOfRawData, &dwNumberOfBytesWritten, NULL);
			}
			CloseHandle(hFile);
		}
		else
